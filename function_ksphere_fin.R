...
ksphere.conform2 <- function(train, test, k, a = 0.05){
  ###kmeans on y1
  ...
  #유의수준
  t = matrix(rep(0,length(a)),nc = length(a))
  cluster.alpha = matrix(rep(c(train.res[,2],metric.res[,2]),length(a)),
                         nr = tr_n + n,nc = length(a))
  ind_tr = 1:tr_n
  ind_te = (tr_n+1):(tr_n+n)
  for(j in 1:length(a)){
    
    t[j] = quantile(metric.res[,1],1-a[j])
    cluster.alpha[ind_te,j] <- sapply(1:n,function(x){ifelse(metric.res[x,1]>t[j],
                                                                 0,metric.res[x,2])})

    cluster.alpha[ind_tr,j] <- sapply(1:tr_n,function(x){ifelse(train.res[x,1]>t[j],
                                                            0,train.res[x,2])})
  }
  ###restore radius
  ...
  for(j in 1:length(a)){
    for(i in 1:k){
      radius[i,j] = sqrt(max(t[j] + 2*log(py[i]) - 
...
print.ksphere <- function(x){
  if(class(x) == "ksphere") {print(list(cluster.train = x[['cluster.train']],
                                        cluster.test = x[['cluster.test']],
                                        nclust = length(unique(x[['merge']]))))}
}
